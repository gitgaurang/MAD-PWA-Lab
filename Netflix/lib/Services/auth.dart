...
class FireBaseServices {
  ...
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
   ...
   Future<void> addSearchQuery(String query) async {
    // Add the search query to the 'search_queries' collection
    await _firestore.collection('search_queries').add({
      'query': query,
      'timestamp': FieldValue.serverTimestamp(), // Store server timestamp
      'userId': _auth.currentUser!.uid, // Store user ID to associate the query with the user
    });
  }
              ...
              "WatchList": FieldValue.arrayRemove([
                {
                  ...
                  // "movieName": watchlist[i]["movieName"],
                  // "moviePhotoUrl": watchlist[i]["moviePhotoUrl"],
                }
              ])
            })
            .then((value) {})
